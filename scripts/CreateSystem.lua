    ...
    for _, entity in pairs(self.targets) do
        ...
        if color == nil then
            ...
            love.graphics.rectangle(
                "fill",
                entity:get("transform").x,
                entity:get("transform").y,
                entity:get("transform").width,
                entity:get("transform").height
            )
                ...
                if entityA ~= entityB then
                    ...
                    if entityA:get("jump").rayActive == true then
                        ...
                        if not colliderA.isTouchingLeftWall then
                            rayA = {
                                origin = {x = transformA.x, y = transformA.y + transformA.height + 5},
                                final = {x = transformA.x, y = transformA.y + transformA.height + offset}
                            }

                            rayACollision =
                                ((rayA.origin.x > transformB.x and rayA.origin.x < transformB.x + transformB.width) and
                                (rayA.final.x > transformB.x and rayA.final.x < transformB.x + transformB.width)) and
                        ...
                        if not colliderA.isTouchingRightWall then
                            rayB = {
                                origin = {
                                    x = transformA.x + transformA.width,
                                    y = transformA.y + transformA.height + 5
                                },
                                final = {
                                    x = transformA.x + transformA.width,
                                    y = transformA.y + transformA.height + offset
                                }
                            }

                            rayBCollision =
                                ((rayB.origin.x > transformB.x and rayB.origin.x < transformB.x + transformB.width) and
                                (rayB.final.x > transformB.x and rayB.final.x < transformB.x + transformB.width)) and
                        ...
                        if rayACollision or rayBCollision then
                            if colliderA.isColliding == false then
                                ...
                                if entityA:get("velocity").y ~= 0 then
                    ...
                    rayC = {
                        origin = {x = transformA.x, y = transformA.y - 5},
                        final = {x = transformA.x, y = transformA.y - offset}
                    }

                    rayCCollision =
                        (rayC.origin.x > transformB.x and rayC.origin.x < transformB.x + transformB.width) and
                        (rayC.origin.y < transformB.y + transformB.height and rayC.origin.y > transformB.y) and
                        (rayC.final.x > transformB.x and rayC.final.x < transformB.x + transformB.width) and
                        (rayC.final.y < transformB.y + transformB.height and rayC.final.y > transformB.y)

                    rayD = {
                        origin = {x = transformA.x + transformA.width, y = transformA.y - 5},
                        final = {x = transformA.x + transformA.width, y = transformA.y - offset}
                    }

                    rayDCollision =
                        (rayD.origin.x > transformB.x and rayD.origin.x < transformB.x + transformB.width) and
                        (rayD.origin.y < transformB.y + transformB.height and rayD.origin.y > transformB.y) and
                        (rayD.final.x > transformB.x and rayD.final.x < transformB.x + transformB.width) and
                        (rayD.final.y < transformB.y + transformB.height and rayD.final.y > transformB.y)
                    ...
                    if (direction.left and not direction.right) then
                        if
                            (transformA.x - transformA.width > transformB.x and
                                transformA.x - transformA.width < transformB.x + transformB.width) and
                                (transformA.y > transformB.y and
                                    transformA.y + transformA.height < transformB.y + transformB.height) and
                                (transformA.x > transformB.x and transformA.x < transformB.x + transformB.width) and
                                (transformA.y > transformB.y and
                         ...
                         then
                            ...
                            transformA.x =
                                transformA.x + (transformB.x + transformB.width) - (transformA.x + transformA.width) +
                                transformA.width +
                                0.4
                            entityA:get("hitComponent").hit = false
                            --entityA:get("jump").rayActive = true
                    ...
                    elseif (direction.right and not direction.left) then
                        if
                            (transformA.x + transformA.width * 2 > transformB.x and
                                transformA.x + transformA.width * 2 < transformB.x + transformB.width) and
                                (transformA.y > transformB.y and
                                    transformA.y + transformA.height < transformB.y + transformB.height) and
                                (transformA.x + transformA.width > transformB.x and
                                    transformA.x + transformA.width < transformB.x + transformB.width) and
                ...
                if attackComponent.charging == true and attackComponent.chargingTime < attackComponent.chargingMaxTime then
                    ...
                    if attackComponent.alreadyScaled == false then
                        transform.x, transform.y =
                            transform.x + attackComponent.minWidth / 2,
                            transform.y + attackComponent.maxWidth - attackComponent.minWidth
                    ...
                    attackComponent.angle =
                        Lerp(minAngle, maxAngle, attackComponent.chargingTime / attackComponent.chargingMaxTime)
                ...
                elseif attackComponent.charging == false and attackComponent.chargingTime > 0 then
                    if GetNumberOfPlayers() > 1 then
                        ...
                        for _, targetEntity in pairs(self.targets) do
                            if
                                transform.name ~= targetEntity:get("transform").name and
                                    targetEntity:get("attackComponent") ~= nil
                             then
                        ...
                        local targetVector = {
                            x = closestPlayerTransform.x + closestPlayerTransform.width / 2,
                        ...
                        local closestDistance =
                            CalculateModule(playerVector.x, playerVector.y, targetVector.x, targetVector.y)
                        ...
                        if closestDistance < distanceFromOther then
                            ...
                            local totalForce =
                                jump.force * math.abs(math.sin(math.rad(attackComponent.angle))) * jumpMultiplier
                            ...
                            if totalForce > -4 then
                                closestPlayer:get("velocity").y = -4
                            else
                                closestPlayer:get("velocity").y = totalForce
                            end
                            ...
                            if transform.x >= closestPlayerTransform.x then
                                ...
                                if debugMode then
                                    print(
                                        "He lanzado a " ..
                                            closestPlayerTransform.name ..
                                                " estando a " ..
                                                    closestDistance ..
                                                        " metros de distancia, a un angulo de " ..
                                                            attackComponent.angle .. " grados a la IZQUIERDA"
                                    )
                                end
                            else
                                        ...
                                        "He lanzado a " ..
                                            closestPlayerTransform.name ..
                                                " estando a " ..
                                                    closestDistance ..
                                                        " metros de distancia, a un angulo de " ..
                                                            attackComponent.angle .. " grados a la DERECHA"
                                    )
                                end
                            end
                        end
                    ...
                    if collider.isTouchingLeftWall then
                        transform.x = transform.x + transform.width / 2
                    elseif collider.isTouchingRightWall then
                        transform.x = transform.x - transform.width / 2
                    end
                    ...
                    for index, wallEntity in pairs(ReturnAllWallEntities()) do
                        ...
                        local newDistance =
                            (transformWall.x + transformWall.width / 2 - (transform.x + transform.width / 2))
                    ...
                    if distance < distanceFromWall then
                        ...
                        local totalForce =
                            jumpComponent.force * math.abs(math.sin(math.rad(attackComponent.angle))) * jumpMultiplier
                        hitComponent.hit = true

                        if totalForce > -4 then
                            velocityComponent.y = -4
                        else
                            velocityComponent.y = totalForce
                        end

                        if playerOrigin >= wallOrigin then
                            hitComponent.angle = attackComponent.angle
                        else
                            hitComponent.angle = attackComponent.angle + 180
                        end
                    end
        ...
        if player:get("hitComponent") ~= nil then
            ...
            if hitComponent.hit == true then
                transformComponent.x =
                    transformComponent.x +
                    math.cos(math.rad(hitComponent.angle)) * velocityComponent.x * horizontalMultiplier * dt
            end
        end
    end
end
        ...
        if entity:get("dragableComponent") ~= nil then
            ...
            if mouseClicked == true then
                if dragable.enabled == true then
                    if
                        mouseX > transform.x and mouseX < transform.x + transform.width and mouseY > transform.y and
